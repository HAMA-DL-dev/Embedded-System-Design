...
#define SUCCESS 0
#define DEVICE_NAME "gpio_button" // Dev name as it appears in /proc/devices   
...
#define GPIO_NUMBER_L 6
#define GPIO_NUMBER_R 24
...
{
    int bytes_read = 1;
	buffer[0] = (char)(readl(addr+GPLEV0) >> GPIO_NUMBER_L) & 0x01 ;
	buffer[1] = (char)(readl(addr+GPLEV0) >> GPIO_NUMBER_R) & 0x01 ;
	return bytes_read;
}
...
static ssize_t device_write(struct file *filp, const char *buff, size_t len, loff_t * off)
{
	...
	if (string[0] !=0 ) {
		gpio_clr(GPIO_NUMBER_L);
		gpio_clr(GPIO_NUMBER_R);
	} else {
		gpio_set(GPIO_NUMBER_L);
		gpio_set(GPIO_NUMBER_R);
	}
	return len;
}
...
int init_module(void)
{
	...
	gpio_input(GPIO_NUMBER_L);
	gpio_input(GPIO_NUMBER_R);
	return SUCCESS;
}
